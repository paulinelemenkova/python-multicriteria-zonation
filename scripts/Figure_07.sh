...
g.region raster=dem
r.slope.aspect elevation=dem slope=slope aspect=aspect --overwrite --quiet
r.out.gdal -f -c input=slope output=slope5254.tif format=GTiff type=Float32 \
...
gdalwarp -overwrite -s_srs EPSG:5254 -t_srs EPSG:4326 -te 26.0 39.0 31.0 42.0 \
         -tr 0.0015 0.0015 -r bilinear -dstnodata -9999 slope5254.tif slope_ll.tif
gmt grdconvert slope_ll.tif slope_ll.nc=nf

gmt grdlandmask -Rslope_ll.nc -Di -N0/1/0/1/0 -Gmask.nc
gmt grdmath mask.nc 0 NAN = mask_nan.nc
gmt grdmath slope_ll.nc mask_nan.nc MUL = slope_masked.nc
...
gmt makecpt -Cturbo -T0/20/0.5 -Z > slope.cpt
echo "N	gray85" >> slope.cpt

gmt begin Figure_07 pdf,png
...
26 42" | gmt plot -Ggray85 -L
  gmt grdimage slope_masked.nc -Cslope.cpt -Q
  ...
  gmt colorbar -Cslope.cpt -DjTR+w6c/0.4c+o0.4c/0.4c+h+e -Ba5f2.5 -F+gwhite+p0.5p
  echo "0.811 0.985 Slope (Degrees)" | gmt text -R0/1/0/1 -JX18c/14.152c -F+f10p,Helvetica-Bold+jBC -N
...
0.005 0.9 Slope, SRTM 3 arc-sec (GMT earth_relief_03s), computed via r.slope.aspect (GRASS). Provinces: geoBoundaries.
EOF
gmt end

echo "Done: Figure_07.pdf and Figure_07.png written to $(pwd)"
